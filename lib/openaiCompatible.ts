...
/**
 * Factory for any OpenAI-compatible provider (OpenAI, Groq, OpenRouter, …).
 * They share the same Chat Completions API shape, so a single implementation
 * parameterized by base URL + model covers all of them.
 */
export function makeOpenAICompatible(opts: {
  ...
  const { model, baseURL, providerLabel } = opts;

  function getClient(apiKey: string) {
    if (!apiKey)
      throw new Error(`No ${providerLabel} API key provided. Please add it in Settings.`);
...
Return ONLY a valid JSON object with a "questions" array, no markdown:
{"questions": [{"question": "…?", "options": ["A","B","C","D"], "correctAnswer": 0, "explanation": "why"}]}
